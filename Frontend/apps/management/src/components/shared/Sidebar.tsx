...
export const Sidebar: React.FC = () => {
  ...
  /* Legacy dark sidebar commented out below for reference:
  return (
    <aside className="w-64 border-r border-slate-800 bg-slate-900/60 ...">
  */

  return (
    <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md min-h-[calc(100vh-57px)] p-4 flex flex-col justify-between transition-colors duration-200">
      <div className="space-y-1">
        <div className="px-3 mb-3 flex items-center justify-between">
          <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Navigation</span>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
            activeRole === ManagementRole.APPROVER ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-cyan-500/20 text-cyan-600 dark:text-cyan-400'
          }`}>
            {activeRole}
          </span>
        </div>
      ...
      <div className="p-3.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700">
        <p className="text-xs font-bold text-cyan-600 dark:text-cyan-400">Security Encrypted</p>
        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Role-gated action approvals & audit logging enabled.</p>
      </div>
    </aside>
  );
};
